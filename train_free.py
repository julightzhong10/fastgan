...
from tools.others import sample_ZCs,make_bin_label,x_R_proccess_free, init_list, PGD_FREE_TYPE
from tools.metric import get_metrics, set_hinge_bd
from tools.adv_tools.pgd_free import l2_cliper
...
parser.add_argument('--itr_g', type=int, required=True, help = 'efault to 1')
...
parser.add_argument('--sa_d', action='store_true', default=False, help = 'if use self-attetion in D')
parser.add_argument('--sa_g', action='store_true', default=False, help = 'if use self-attetion in G')
...
parser.add_argument('--pgd_free_steps', type=int, required=True, help = 'number of steps for pgd free')
...
parser.add_argument('--pgd_tau', type=float, required=True, help = 'lr of sgd for pgd attack')
...
def train():
    ...
    tpts = time.time()
    itrs = 0
    delta = torch.zeros([opt.r_batch_size, opt.img_c, opt.img_width, opt.img_width]) # free accumulated perturbation
    if opt.n_gpu>0:
        delta = delta.cuda()
    if opt.pgd_rs: # perturbation init
        delta.normal_()
        if opt.pgd_type==PGD_FREE_TYPE[0]:
            delta.clamp_(-opt.pgd_eps,opt.pgd_eps)
        elif opt.pgd_type==PGD_FREE_TYPE[1]:
            delta = l2_cliper(delta,opt.pgd_eps,opt.n_gpu)
    delta = list(torch.chunk(delta,opt.num_bp))
    ...
    while G_itrs<opt.G_total_itrs:
        ...
        if opt.g_batch_size%opt.num_bp !=0:
            g_BS[-1] = opt.g_batch_size%opt.num_bp
        ...
        if itrs%opt.itr_g==0:
            ...
            x_F, y_F_c = init_list(opt.num_bp),init_list(opt.num_bp)
            for bp_i in range(opt.num_bp):
                # produce fake imgs from G
                Z, C_int, C_vec = sample_ZCs(g_BS[bp_i],opt.dim_z,opt.n_classes,Z_dist,Z_params,opt.n_gpu) # z <- Normal(0,1)
                x_F[bp_i] = netG(Z,C_vec).detach() # x_F <- G(z)
                y_F_c[bp_i] = C_int.view(-1)
                
            for free_step in range(opt.pgd_free_steps):
                D_cnt += 1
                netD.zero_grad()
                for bp_i in range(opt.num_bp):
                    # g_xR, g_wDr <- D(x_R)
                    delta[bp_i], loss_dis_R = x_R_proccess_free(netD=netD,x_R=x_R[bp_i],y_R_b=y_R_b[bp_i],y_R_c=y_R_c[bp_i],metric = Metric_R,\
                                            delta=delta[bp_i], pgd_type=opt.pgd_type, pgd_eps=opt.pgd_eps, pgd_tau=opt.pgd_tau,\
                                            b_metric=opt.b_metric,c_metric=opt.c_metric, c_alph=opt.c_alph_r,n_gpu=opt.n_gpu,num_bp=opt.num_bp)
                    D_err_R += (loss_dis_R.item())
                    o_F_b,o_F_c = netD(x_F[bp_i],y_F_c[bp_i]) 
                    loss_dis_F = Metric_F(o_F_b,o_F_c,y_F_b[bp_i],y_F_c[bp_i],opt.b_metric,opt.c_metric,1.0,opt.c_alph_f) 
                    loss_dis_F = (loss_dis_F/opt.num_bp) 
                    loss_dis_F.backward() # g_wDf <- D(x_F)
                    D_err_F += (loss_dis_F.item())
                optimizerD.step()  # wD <- wD - (g_wDf + g_wDr) * lr
        ...
        if itrs % opt.itr_d == 0:
            ...
            for bp_i in range(opt.num_bp):
                ...
                G_err += loss_gen.item()
                loss_gen.backward() # g_wG <- D(G(z))
            optimizerG.step() # wG <- wG - g_wG  * lr
        ...
        # finally add itrs for keeping count continuing
        itrs += 1 
