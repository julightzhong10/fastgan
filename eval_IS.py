...
def eval():
    ...
    if opt.real_data_IS:
        train_loader,class_to_idx = load_datasets(data_path=opt.data_path,img_width=opt.img_width,\
        ...
        if opt.tf:
                ...
                inception_model = load_inception_IS(opt.n_gpu)        
        for i in range(opt.start,opt.end+1,opt.skip):
                curr_path = f"{opt.load_path}gen_{i}.pth"
                ...
                print(f"{[round(IS_mean.item(),4),round(IS_std.item(),4),curr_path]}")
