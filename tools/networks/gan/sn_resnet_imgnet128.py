...
paras:       
    w/o sa: netD:30,042,857, netG:41,996,931
    w/  sa: netD:30,063,338, netG:42,078,852,
...
class SN_ResNet_ImgNet128_Dis(nn.Module):
    def __init__(self, img_c=3,fm_base=64, n_classes=0, bn=False, sn=False,sa=False,c_metric='',wide=False):
        ...
        self.block6 = DownBlock(fm_base * 16, fm_base * 16, (fm_base*16 if self.wide else fm_base*16) , activation=self.activation, downsample=False, bn=bn, sn=sn)
        # bin
...
class SN_ResNet_ImgNet128_Gen(nn.Module):
    def __init__(self, dim_z=128,img_c=3,fm_base=64, bottom_width=4, n_classes=0, bn=False, sn=False,sa=False):
        ...
        self.block2 = UpBlock(fm_base * 16, fm_base * 16, activation=self.activation, upsample=True, n_classes=n_classes,bn=bn, sn=sn)
        self.block3 = UpBlock(fm_base * 16, fm_base * 8, activation=self.activation, upsample=True, n_classes=n_classes,bn=bn, sn=sn)
        self.block4 = UpBlock(fm_base * 8, fm_base * 4, activation=self.activation, upsample=True, n_classes=n_classes,bn=bn, sn=sn)
        if self.sa:
