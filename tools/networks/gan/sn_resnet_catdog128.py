...
paras:       
    ...
    w/o sa: netD:8,583,568, netG:10,606,339
    w/  sa: netD:8,588,769, netG:10,606,339
...
class SN_ResNet_CatDog128_Dis(nn.Module):
    def __init__(self, img_c=3,fm_base=64, n_classes=0, bn=False, sn=False,sa=False,c_metric='',wide=False):
        ...
        self.block2 = DownBlock(fm_base, fm_base * 2, (fm_base*2 if self.wide else fm_base), activation=self.activation, downsample=True, bn=bn, sn=sn)
        if self.sa:
        ...
        self.block4 = DownBlock(fm_base * 4, fm_base * 4, (fm_base*4 if self.wide else fm_base*4), activation=self.activation, downsample=True, bn=bn, sn=sn)
        self.block5 = DownBlock(fm_base * 4, fm_base * 8, (fm_base*8 if self.wide else fm_base*4), activation=self.activation, downsample=True, bn=bn, sn=sn)
        self.block6 = DownBlock(fm_base * 8, fm_base * 8, (fm_base*8 if self.wide else fm_base*8), activation=self.activation, downsample=False, bn=bn, sn=sn)
        # bin
        self.o_b = nn.Linear(fm_base * 8, 1)
        nn.init.xavier_uniform_(self.o_b.weight)
        # class 
        if n_classes > 0:
            if c_metric in [Metric_Types[0],Metric_Types[2]]:
                self.o_c = nn.Linear(fm_base * 8, n_classes)   
            elif c_metric == Metric_Types[1]:
                self.o_c = nn.Embedding(n_classes, fm_base * 8)
...
class SN_ResNet_CatDog128_Gen(nn.Module):
    def __init__(self, dim_z=128,img_c=3,fm_base=64, bottom_width=4, n_classes=0, bn=False, sn=False,sa=False):
        ...
        self.n_classes = n_classes
        self.sa = sa
        self.l1 = nn.Linear(dim_z, (bottom_width ** 2) * fm_base * 8)
        nn.init.xavier_uniform_(self.l1.weight)
        self.block2 = UpBlock(fm_base * 8, fm_base * 8, activation=self.activation, upsample=True, n_classes=n_classes,bn=bn, sn=sn)
        self.block3 = UpBlock(fm_base * 8, fm_base * 4, activation=self.activation, upsample=True, n_classes=n_classes,bn=bn, sn=sn)
        self.block4 = UpBlock(fm_base * 4, fm_base * 4, activation=self.activation, upsample=True, n_classes=n_classes,bn=bn, sn=sn)
        if self.sa:
            self.attn1 = Attention(ch=fm_base * 4,sn=sn)
        self.block5 = UpBlock(fm_base * 4, fm_base * 2, activation=self.activation, upsample=True, n_classes=n_classes,bn=bn, sn=sn)
        self.block6 = UpBlock(fm_base * 2, fm_base * 1, activation=self.activation, upsample=True, n_classes=n_classes,bn=bn, sn=sn)
        if bn:
